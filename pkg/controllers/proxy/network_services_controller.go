...
const (
	...
	networkServiceControllerLabel = "NSC"
)
...
func (nsc *NetworkServicesController) Run(stopCh <-chan struct{}, wg *sync.WaitGroup) error {
	...
	for {
		...
		if err != nil {
			...
			healthcheck.SendHeartBeat(nsc.healthChan, networkServiceControllerLabel)
		}
...
func (nsc *NetworkServicesController) sync() error {
	...
	if err != nil {
		metrics.ControllerErrors.WithLabelValues(networkServiceControllerLabel).Inc()
...
func (nsc *NetworkServicesController) syncIpvsServices(serviceInfoMap serviceInfoMap, endpointsInfoMap endpointsInfoMap) error {
	...
	for k, svc := range serviceInfoMap {
		...
		for _, endpoint := range endpoints {
			...
			if err != nil {
				metrics.ControllerErrors.WithLabelValues(networkServiceControllerLabel).Inc()
				glog.Errorf("Error adding clusterIP server to IPVS service %s", err.Error())
			}
			...
			for _, externalIpService := range externalIpServices {
				...
				if err != nil {
					metrics.ControllerErrors.WithLabelValues(networkServiceControllerLabel).Inc()
					glog.Errorf("Error adding externalIP server to IPVS service %s", err.Error())
				}
				...
				if svc.directServerReturn && svc.directServerReturnMethod == "tunnel" {
					...
					if err != nil {
						metrics.ControllerErrors.WithLabelValues(networkServiceControllerLabel).Inc()
						glog.Errorf("Failed to find endpoint with ip: " + endpoint.ip + ". so skipping preparing endpoint for DSR")
						continue
					}
...
func NewNetworkServicesController(clientset kubernetes.Interface,
	config *options.KubeRouterConfig, healthChan chan<- *healthcheck.ControllerHeartbeat, svcInformer cache.SharedIndexInformer,
